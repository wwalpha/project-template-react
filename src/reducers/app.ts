...
const app = handleActions<AppState, any>(
  {
    ...
    [ActionTypes.APP_PLUS_SUCCESS]: (state: AppState, { payload: { num } }: Action<App01Payload>) =>
      produce(state, (draft) => {
        draft.count += num;
      }),
    ...
    [ActionTypes.APP_MINUS_SUCCESS]: (state: AppState, { payload: { num } }: Action<App02Payload>) =>
      produce(state, (draft) => {
        draft.count -= num;
      }),
